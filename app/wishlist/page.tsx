...
export default function WishlistPage() {
  ...
  const loadWishlist = async () => {
    ...
    setProducts(saved.flatMap(({ products: product }) => {
      if (!product) return [];
  ...
  return <main className="min-h-screen bg-[#f7f7f5] text-black">
    <header className="flex h-16 items-center justify-between border-b border-black/10 px-5 sm:px-8 lg:px-12"><Link href="/" className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.17em]"><span className="grid size-7 place-items-center rounded-full border border-black/20 text-base font-normal">←</span>Collection</Link><Link href="/" className="absolute left-1/2 -translate-x-1/2 text-[16px] font-black tracking-[-.08em]">DRIP<span className="font-normal">NALITY</span><sup className="ml-0.5 text-[6px]">®</sup></Link><Link href="/account" className="text-[10px] font-bold uppercase tracking-[.17em]">Account</Link></header>
    <section className="mx-auto max-w-[1400px] px-5 py-16 sm:px-8 lg:px-12 lg:py-24"><p className="eyebrow">Personal archive</p><h1 className="mt-5 text-[clamp(3.5rem,9vw,9rem)] font-black leading-[.77] tracking-[-.1em]">SAVED<br />PIECES.</h1>{loading ? <p className="mt-16 text-[10px] font-bold uppercase tracking-[.18em] text-black/45">Loading saved pieces</p> : products.length ? <div className="mt-16 grid gap-6 md:grid-cols-2">{products.map((product) => <article key={product.id}><div className="relative aspect-[4/4.3] bg-black/[.04]"><Image src={product.image} alt={product.name} fill sizes="(max-width:768px) 100vw,50vw" className="object-contain" /><button onClick={() => removeSaved(product)} className="absolute right-4 top-4 bg-white px-3 py-2 text-[9px] font-bold tracking-[.13em] transition hover:bg-black hover:text-white">REMOVE</button><button onClick={() => setSelectedProduct(product)} className="absolute inset-x-4 bottom-4 flex justify-between bg-white px-4 py-3 text-[10px] font-bold tracking-[.14em]">QUICK VIEW <span>↗</span></button></div><div className="mt-4 flex justify-between gap-5"><div><h2 className="text-[12px] font-bold uppercase">{product.name}</h2><p className="mt-1 text-[10px] text-black/50">{product.color}</p></div><p className="text-[12px] font-bold">${product.price}.00</p></div></article>)}</div> : <div className="mt-14 border-t border-black/15 pt-8"><p className="max-w-sm text-sm leading-6 text-black/55">Your saved archive is empty. Use the heart on any piece to keep it here.</p><Link href="/#collection" className="mt-7 inline-flex bg-black px-5 py-3 text-[10px] font-bold uppercase tracking-[.15em] text-white">Explore hoodies →</Link></div>}</section><Footer /><QuickView product={selectedProduct} onClose={() => setSelectedProduct(null)} onAdd={() => undefined} /></main>;
}
